...
function GSaveData(){
    ...
    this.saveAll = function(){
        IRWFile.SaveKV(RV.NowProject.key + "_saveInfo",_sf.list);
    };
...
function IMain(){
    ...
    this.endInterpreter = function(){
        RV.GameData.storySelectInfo = [];
        RV.GameData.iIndex = -1;
        RV.GameData.ievent = -1;
        this.subStory = null;
    ...
    function poaAdd(){
        ...
        if(_sf == RV.InterpreterMain) RV.GameData.iIndex = _sf.pos;
        if(_sf == RV.InterpreterMain) RV.GameData.ievent = _sf.event_list[_sf.pos].code;
...
function LCanvas(){
    ...
    this.load = function(){
        ...
        for(i = 0;i<animSet.length;i++){
            var oldX = animSet.oldX;
            var oldY = animSet.oldY;
            var rect = new IRect(oldX , oldY , oldX + 10, oldY + 10);
            var am = RV.NowCanvas.playAnim(animSet.resId,null,null,animSet.single,rect,animSet.id);
            if(am != null){
                am.resId = animSet.resId;
                am.single = animSet.single;
                am.id = animSet.id;
                am.oldX = animSet.oldX;
                am.oldY = animSet.oldY;
            }
        }
...
function LMessage(){
    ...
    function initMsg(){
        ...
        _sf.buttonSkip = ctrl.buttonSkip;
        _sf.buttonAuto = ctrl.buttonAuto;
        _sf.buttonMenu = ctrl.buttonMenu;
        ...
        ctrl.x = RV.NowProject.gameWidth + ((RV.NowProject.gameWidth - ctrl.width) / 2);
        ctrl.y = RV.NowProject.gameHeight;
        _sf.pt = ctrl.tips;
        ...
        _sf.buttonSkip.setBitmap(RF.LoadCache("System/" + _sf.buttonSkip.data.pic1),RF.LoadCache("System/" + _sf.buttonSkip.data.pic1),true);
        _sf.buttonAuto.setBitmap(RF.LoadCache("System/" + _sf.buttonAuto.data.pic1),RF.LoadCache("System/" + _sf.buttonAuto.data.pic1),true);
        _sf.buttonMenu.setBitmap(RF.LoadCache("System/" + _sf.buttonMenu.data.pic1),RF.LoadCache("System/" + _sf.buttonMenu.data.pic1),true);
    ...
    this.setThis = function( b,pointX,pointY){
        ...
        if(pointX == "a"){
            ...
            _sf.viewport.y = by +((RV.NowProject.gameHeight - ctrl.height) / 2);
        }else if(pointX == "c"){
            _sf.viewport.y = by + (RV.NowProject.gameHeight - ctrl.height - 20);
        }else{
    ...
    function updatePCKey(){
        if(IInput.isKeyDown(RC.Key.skip)){
            _sf.isClick = true;
            if(_sf.buttonSkip.tag == 0){
                _sf.buttonSkip.setBitmap(RF.LoadCache("System/" + _sf.buttonSkip.data.pic2),RF.LoadCache("System/" + _sf.buttonSkip.data.pic2),true);
                _sf.buttonSkip.tag = 1;
            }else{
                _sf.buttonSkip.setBitmap(RF.LoadCache("System/" + _sf.buttonSkip.data.pic1),RF.LoadCache("System/" + _sf.buttonSkip.data.pic1),true);
                _sf.buttonSkip.tag = 0;
            }
        ...
        if(IInput.isKeyDown(RC.Key.auto)){
            _sf.isClick = true;
            if(_sf.buttonAuto.tag == 0){
                _sf.buttonAuto.setBitmap(RF.LoadCache("System/" + _sf.buttonAuto.data.pic2),RF.LoadCache("System/" + _sf.buttonAuto.data.pic2),true);
                _sf.buttonAuto.tag = 1;
            }else{
                _sf.buttonAuto.setBitmap(RF.LoadCache("System/" + _sf.buttonAuto.data.pic1),RF.LoadCache("System/" + _sf.buttonAuto.data.pic1),true);
                _sf.buttonAuto.tag = 0;
            }
    ...
    function updateButton(){
        ...
        if(_sf.buttonMenu.update()){
            ...
            _sf.isClick = true;
            RV.GameData.menu = -1;
            //var id = IVal.scene.getMenuId();
            //IVal.scene.callMenu(id);
            //return;
        }
    ...
    function drawName( name){
        ...
        var fontSize = _sf.nameDraw.data.fontColor.TColor() + "\\s["+ _sf.nameDraw.data.fontSize + "]";
        LUI.setText(_sf.nameDraw,fontSize + RF.MakerValueText(name));
    ...
    this.updateDraw = function(){
        ...
        while (true) {
            if(drawText == null || drawText.length <= 0 && drawIndex + 1 < textGroup.length){
                ...
                if(_sf.dx + w >= this.talkDraw.width - (this.talkDraw.data.x * 2) - rdw){
            ...
            if(c == 60000){//换行
                ...
                this.talkDraw.drawText(color.TColor() + "\\s[" + fontSize + "]" + min, _sf.dx, _sf.dy, this.talkDraw.data.textStyle, this.talkDraw.data.textStyleColor.IColor() , false, 0);
                //this.talkDraw.drawTextQ(min, _sf.dx, _sf.dy, color, fontSize);
                _sf.dx += IFont.getWidth(min,fontSize) * spacingH;
                if(_sf.dx > this.talkDraw.width - (this.talkDraw.data.x * 2) - rdw){
                    _sf.dx = rdx;
                    _sf.dy += (IFont.getHeight(min, fontSize) * spacingV);
                }
            }
 ...
 */
RF.IsNext = function(){
    return IInput.up || IInput.isKeyDown(13) || IInput.isKeyDown(27) || IInput.isKeyDown(32);
};
/**
 * 默认颜色 0号
 * @returns {IColor}
    ...
    this.updateFile = function(ctrl,obj){
        ...
        if(obj != tempObj){
            ...
            if(_sf.currentList[_sf.selectIndex].own == true){
                var data = RV.SaveInfo.load(_sf.selectIndex);
                recoverPic(data);
            }else{
                disposePic()
            }
...
function WSave(ui){
    ...
    this.updateSave = function(index){
        ...
        var data = RV.SaveInfo.load(index);
        recoverPic(data);
        var selfIndex = 0;
        if(index < 10){
            selfIndex = "0"+ index
